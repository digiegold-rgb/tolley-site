...
export default function ProbatePage() {
  ...
  async function updateStatus(id: string, status: string) {
    ...
    setNotice(null);
    try {
      const res = await fetch(`/api/serpapi/probate/${encodeURIComponent(id)}`, {
        method: "PATCH",
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-white/40">
            Review obituary sources, candidate heirs, and possible property matches.
            An obituary is not a verified probate filing or proof of intent to sell.
            Add an opportunity to Today to save a lead and a private verification task.
          </p>
        </div>
        <button
          onClick={triggerScan}
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  {s.matchedAddress && <p className="mt-2 text-sm text-white/70">Possible property: {s.matchedAddress}</p>}
                  {s.heirsJson && s.heirsJson.length > 0 && (
                    <div className="mt-2 text-xs text-white/70">
                      <span className="text-white/40">Candidate heirs:</span>{" "}
                      {s.heirsJson
                        .map((h) =>
                ...
                <div className="flex flex-col items-end gap-1">
                  ...
                  <div className="mt-1 flex flex-wrap justify-end gap-2">
                    ...
                    {s.status === "dismissed" && <button disabled={saving !== null} onClick={() => updateStatus(s.id, "discovered")} className="text-xs text-white/60 underline">Restore for review</button>}
