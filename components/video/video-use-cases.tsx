import { VIDEO_USE_CASES } from "@/lib/video";
  ...
  return (
    <section>
      ...
      <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {VIDEO_USE_CASES.map((uc) => (
          <div key={uc.title} className="vid-card flex gap-4 rounded-xl border border-purple-400/15 bg-purple-400/[0.04] p-5">
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg border border-purple-400/20 bg-purple-500/10 text-purple-300">
              {ICONS[uc.icon]}
            </div>
            <div>
              <h3 className="font-bold text-white">{uc.title}</h3>
              <p className="mt-1 text-sm leading-relaxed text-slate-400">{uc.description}</p>
            </div>
          </div>
        ))}
      </div>
