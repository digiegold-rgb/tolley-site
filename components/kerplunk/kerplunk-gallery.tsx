...
import { KP_IMAGES } from "@/lib/kerplunk";

const images: readonly string[] = KP_IMAGES;

export function KerplunkGallery() {
  ...
  return (
    <section className="rounded-xl border border-slate-700/50 bg-[#180d28] p-4 sm:p-6">
      <h2 className="kp-neon-text mb-4 text-2xl font-black tracking-wide text-[#e040a0] uppercase sm:text-3xl">
        Photos
      </h2>
      ...
      <div className="relative aspect-[4/3] w-full overflow-hidden rounded-lg bg-black/30">
        <Image
          src={images[idx]}
          alt={`Giant Kerplunk photo ${idx + 1}`}
          fill
          ...
          {images.map((src, i) => (
            <button
              ...
              className={`relative h-14 w-20 flex-shrink-0 overflow-hidden rounded border-2 transition sm:h-16 sm:w-24 ${
                i === idx ? "border-[#e040a0]" : "border-transparent opacity-60 hover:opacity-100"
              }`}
            >
