...
import { MV_COMPANY, MV_CONTACT_EMAIL, MV_CONTACT_PHONE, MV_FACEBOOK_URL } from "@/lib/moving";
    ...
    <footer className="relative z-10 mx-auto w-full max-w-6xl px-5 pt-4 pb-10 sm:px-8">
      <div className="border-t border-neutral-800 pt-6">
        <div className="flex flex-col items-center gap-4 text-center sm:flex-row sm:justify-between sm:text-left">
          <div>
            <p className="font-black tracking-wide text-white uppercase">{MV_COMPANY}</p>
            <p className="mt-1 text-sm font-light text-neutral-500">
              <a href={`mailto:${MV_CONTACT_EMAIL}`} className="transition hover:text-emerald-400">
                {MV_CONTACT_EMAIL}
              </a>
              {" \u00B7 "}
              <a href={`tel:${MV_CONTACT_PHONE}`} className="transition hover:text-emerald-400">
                {MV_CONTACT_PHONE}
              </a>
            </p>
          </div>
          <nav className="flex items-center gap-3 text-sm text-neutral-500">
            ...
            <Link href="/trailer/privacy" className="transition hover:text-emerald-400">
              Privacy
            </Link>
            <span className="text-neutral-700">|</span>
            <a
        ...
        <div className="mt-4 flex flex-wrap justify-center gap-3">
          ...
          <div className="rounded-lg border border-yellow-400/15 bg-yellow-400/[0.04] px-4 py-2.5 text-center">
            <p className="text-xs font-bold tracking-wider text-yellow-400/70 uppercase">
              Need power?{" "}
              <Link href="/generator" className="text-yellow-400 transition hover:text-yellow-300">
        ...
        <p className="mt-4 text-center text-sm font-light text-neutral-600">
          Skip the cardboard. Move smarter.
        </p>
      </div>
    </footer>
  );
