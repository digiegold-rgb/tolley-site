...
function deriveSavedResultTitle(queryText: string, answerText: string) {
  const trimmedQuery = queryText.trim();
  if (trimmedQuery) {
    if (trimmedQuery.length <= 80) {
      return trimmedQuery;
    }
    return `${trimmedQuery.slice(0, 77)}...`;
  }
  ...
  if (!firstLine) {
    return "Saved T-Agent Result";
  }

  return firstLine.length <= 80 ? firstLine : `${firstLine.slice(0, 77)}...`;
}

function buildSavedContentText(message: AgentMessage) {
  const parts: string[] = [];
  ...
  if (message.cards?.length) {
    ...
    for (const card of message.cards) {
      const segments: string[] = [card.address];
      if (typeof card.price === "number") {
        segments.push(`$${card.price.toLocaleString("en-US")}`);
      }
      if (typeof card.beds === "number") {
        segments.push(`${card.beds} bd`);
      }
      if (typeof card.baths === "number") {
        segments.push(`${card.baths} ba`);
      }
  ...
  if (message.followUps?.length) {
    ...
    for (const item of message.followUps) {
      parts.push(`- ${item}`);
    }
  }
...
export function TAgentPortal() {
  ...
  const [loginModalMessage, setLoginModalMessage] = useState(
    "Log in with your account credentials to continue searching.",
  );
  ...
  const [savingResultMessageId, setSavingResultMessageId] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    if (authStatus === "authenticated") {
      ...
      setLoginModalMessage("Log in with your account credentials to continue searching.");
    }
  }, [authStatus]);
  ...
  const runSearch = async (queryInput: string) => {
    ...
    if (authStatus !== "authenticated") {
      openLoginModal(
        "Sign In Required",
        "Log in with your account credentials to unlock T-Agent search.",
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok || data?.error) {
        ...
        if (errorCode === "LOGIN_REQUIRED") {
          openLoginModal(
            "Sign In Required",
            "Log in with your account credentials to continue.",
          );
  ...
  const saveResult = async (assistantMessageId: string) => {
    if (authStatus !== "authenticated") {
      openLoginModal("Sign In Required", "Sign in to save results.");
      return;
    }
    ...
    if (assistantIndex < 0) {
      setErrorMessage("Unable to locate result to save.");
      return;
    }
    ...
    const queryText = priorUserMessage?.text || "";

    setSavingResultMessageId(assistantMessageId);
    setErrorMessage(null);

    const payload = {
      query: queryText || "Saved conversation result",
      title: deriveSavedResultTitle(queryText, assistantMessage.text || ""),
      contentJson: {
        query: queryText || null,
        answer: assistantMessage.text || "",
        cards: assistantMessage.cards || [],
        followUps: assistantMessage.followUps || [],
        requestId: assistantMessage.requestId || null,
        cached: assistantMessage.cached ?? null,
        latency: assistantMessage.latency ?? null,
        conversationId,
      },
      contentText: buildSavedContentText(assistantMessage),
    };
    ...
    try {
      ...
      if (!response.ok || data.error || !data.savedResult?.id) {
        setErrorMessage(data.error || "Unable to save result right now.");
        return;
      }

      router.push(`/results/${data.savedResult.id}`);
    } catch {
      setErrorMessage("Unable to save result right now.");
    } finally {
      setSavingResultMessageId(null);
    }
  };
  ...
  return (
    ...
    >
          ...
          {hasConversation ? (
            <>
              ...
              <ChatThread
                ...
                onSaveResult={(messageId) => {
                  void saveResult(messageId);
                }}
                savingListingAddress={savingListingAddress}
                savingResultMessageId={savingResultMessageId}
              />
      ...
      <UsageLimitModal
        ...
        onUpgrade={() => void startCheckout("premium")}
      />
    </main>
  );
}
