...
import { isWdServiceZip, WD_OUT_OF_AREA_MESSAGE } from "@/lib/wd-service-zips";
...
export function WdLeadForm() {
  ...
  const zipBlocked = /^\d{5}/.test(form.zip.trim()) && !isWdServiceZip(form.zip);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.name.trim() || (!form.phone.trim() && !form.email.trim()) || !/^\d{5}$/.test(form.zip.trim())) {
      setError("Enter your name, a phone or email, and a five-digit delivery ZIP.");
      return;
    }
    if (!isWdServiceZip(form.zip) && !form.phone.trim()) {
      setError("");
      return;
    }
    ...
    try {
      ...
      const saved = await submitLead("/api/lead/action", {
      ...
      setOutOfArea(typeof saved.outOfArea === "boolean" ? saved.outOfArea : !isWdServiceZip(form.zip));
      setStatus("sent");
    } catch (err) {
  ...
  return (
    <section className="rounded-2xl bg-white p-6 shadow-lg shadow-blue-100/50 sm:p-8">
      ...
      {zipBlocked && <p role="status" className="mt-3 text-sm leading-relaxed text-slate-700">{WD_OUT_OF_AREA_MESSAGE}</p>}
      ...
      <form onSubmit={handleSubmit} className="mt-5 grid gap-4 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2">
          <button
            type="submit"
            disabled={status === "sending" || (!form.phone && !form.email) || (zipBlocked && !form.phone.trim())}
          ...
          >
            {status === "sending" ? "Sending..." : zipBlocked ? "Leave my number" : "Get My Free Quote"}
          </button>
