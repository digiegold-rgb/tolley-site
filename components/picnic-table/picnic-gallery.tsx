...
import { PT_IMAGES } from "@/lib/picnic-table";

const images: readonly string[] = PT_IMAGES;

export function PicnicGallery() {
  ...
  return (
    <section className="rounded-xl border border-slate-700/50 bg-[#0d1f12] p-4 sm:p-6">
      <h2 className="pt-neon-text mb-4 text-2xl font-black tracking-wide text-[#c4a56e] uppercase sm:text-3xl">
        Photos
      </h2>
          ...
          {images.map((src, i) => (
            <button
              ...
              className={`relative h-14 w-20 flex-shrink-0 overflow-hidden rounded border-2 transition sm:h-16 sm:w-24 ${
                i === idx ? "border-[#c4a56e]" : "border-transparent opacity-60 hover:opacity-100"
              }`}
            >
