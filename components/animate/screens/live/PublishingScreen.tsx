...
export function PublishingScreen(): React.ReactElement {
  ...
  const loadAccounts = React.useCallback(async () => {
    try {
      // Mirror the aggregator first so revoked tokens show as "reconnect".
      try {
        ...
        if (sync.ok) {
          ...
          setVendorEnabled(Boolean(sd.vendor));
        }
      } catch {
  ...
  }, [loadPosts, loadAccounts, loadSocialPosts]);
  ...
  return (
    <div>
      ...
      {flash && (
        ...
        >
          {flash.kind === 'connected' && `✅ ${PLATFORM_META[flash.platform as Platform]?.label ?? flash.platform} connected. You can post to it from any finished video.`}
          ...
          {SUPPORTED_PLATFORMS.map((p) => {
            ...
            return (
              ...
              >
                ...
                <div style={{ fontSize: 11, color: t.textSecondary, marginTop: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {connected
                    ? acc?.username
...
/**
 * Connect / reconnect / disconnect for a platform routed through the
 * aggregator. The OAuth is still to the USER's account (vendor-hosted
 * consent screen); we never see the token.
 */
