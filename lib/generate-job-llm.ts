...
- NSFW chips write prompt AND negative. Allow NSFW strips adult NSFW-block terms from negative_prompt AND injects a durable [[allow-nsfw-wardrobe]] override into prompt (ignore/remove grey shirt and default clothes from identity refs; wardrobe follows the prompt's wardrobe line). Block NSFW re-merges those terms and removes that override block — do not invent moralizing refusal text. extra_image_urls may hold a lingerie/nude body keep-still; clothed identity refs alone keep covering. Chat may still set prompt / negative_prompt / extra_image_urls freely. Never remove child/minor (or other CSAM) terms from negatives.
- Location / Hair / Camera chips write durable [[location]] / [[hair]] / [[camera]] blocks (labeled Location: / Hair: / Camera: lines). If those markers are already on the card, keep them unless Jared asks to change setting, hair, or lens. Do not strip identity-lock sentences. Do not invent Extra-image auto-wiring for wardrobe.
