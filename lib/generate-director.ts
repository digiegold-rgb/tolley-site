...
Chat MAY change seed, steps (num_inference_steps), width, height, true_cfg_scale, guidance_scale, max_sequence_length, num_images, negative_prompt, identity_ref_urls, extra_image_urls, sigmas, pipe_overrides, and prompt when asked — those are job-card / Modal kwargs, not Comfy nodes. pipe_overrides is a rare Diffusers pipe() escape hatch, not Comfy. This recipe has no denoise/strength.`;
