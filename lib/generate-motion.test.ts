...
describe("isFalConfigured + public status", () => {
  it("requires FAL_KEY and never invents Seedance / LatentSync", () => {
    ...
    assert.equal(status.i2v, "alibaba/wan-3.0/image-to-video");
    assert.equal(status.flf2v, "alibaba/wan-3.0/image-to-video");
...
describe("card → fal input", () => {
  it("forwards the source still as start_image_url and no credentials", () => {
    ...
    assert.equal(planned.input.duration, 5);
...
describe("falModelIdFromCardHint", () => {
  it("reads fal_model or end_image_url", () => {
    ...
    assert.equal(falModelIdFromCardHint({ end_image_url: STILL }, "fal-wan-i2v"), "wan30-i2v");
    assert.equal(falModelIdFromCardHint({}, "fal-wan-i2v"), "wan30-i2v");
  });
});
