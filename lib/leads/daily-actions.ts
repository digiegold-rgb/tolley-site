...
export async function applyDailyAction(subscriberId: string, owner: boolean, input: z.infer<typeof dailyActionSchema>, now = new Date()) {
  if (input.action === "outcome") {
    ...
    return await prisma.$transaction(async tx => {
      ...
      const claimed = await tx.crmTask.updateMany({ where: { id: task.id, subscriberId, status: "pending", updatedAt: task.updatedAt }, data: input.outcome === "snooze" ? { dueDate: new Date(input.nextAt!), title: input.nextTitle || task.title, description: input.note || task.description, updatedAt: now } : { status: "completed", completedAt: now } });
      if (!claimed.count) {
      ...
      if (input.outcome !== "snooze" && input.nextAt) await tx.crmTask.create({ data: { subscriberId, ...refs, title: input.nextTitle || `Follow up: ${task.title}`.slice(0,200), description: input.note || task.description, dueDate: new Date(input.nextAt), priority: task.priority } });
      return { saved: true };
    });
    } catch (error) {
