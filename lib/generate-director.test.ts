...
describe("generate route branding", () => {
  it("does not inherit t-agent Real Estate Unlocked on /generate", () => {
    ...
    assert.match(studio, /applyAllowNsfw/);
    assert.match(studio, /applyBlockNsfw/);
    assert.match(studio, /applyLocation/);
    ...
    const locationRow = studio.indexOf('ariaLabel="Location"');
    const hairRow = studio.indexOf('ariaLabel="Hair"');
    const cameraRow = studio.indexOf('ariaLabel="Camera"');
    assert.ok(modalPanel >= 0 && motionForm > modalPanel && blockChip > modalPanel && blockChip < motionForm);
    assert.ok(locationRow > modalPanel && locationRow < motionForm);
    assert.ok(hairRow > modalPanel && hairRow < motionForm);
...
describe("DIRECTOR_SYSTEM_PROMPT headless control", () => {
  it("forbids ComfyUI/node advice and names job-card / Modal kwargs", () => {
    ...
    assert.match(DIRECTOR_SYSTEM_PROMPT, /allow-nsfw-wardrobe/);
    assert.match(DIRECTOR_SYSTEM_PROMPT, /\[\[location\]\]/);
    assert.match(DIRECTOR_SYSTEM_PROMPT, /\[\[hair\]\]/);
    assert.match(DIRECTOR_SYSTEM_PROMPT, /\[\[camera\]\]/);
    assert.match(DIRECTOR_SYSTEM_PROMPT, /Never strip child\/minor/);
  });
});
