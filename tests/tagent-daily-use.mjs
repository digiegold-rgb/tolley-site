...
const match=(row,where={})=>Object.entries(where).every(([k,v])=>{
  ...
  if(v&&typeof v==='object') return Object.entries(v).every(([op,value])=>op==='in'?value.includes(row[k]):op==='notIn'?!value.includes(row[k]):op==='gte'?row[k]>=value:op==='lt'?row[k]<value:op==='startsWith'?row[k]?.startsWith(value):false);
  return row[k]===v;
});
