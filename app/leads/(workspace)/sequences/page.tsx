...
export default function SequencesPage() {
  ...
  const fetchSequences = useCallback(async () => {
    try {
      ...
      toast({
        title: "Network error loading sequences",
  ...
  const handleCreate = async (data: {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        toast({
          title: "Couldn't create sequence",
          description: err?.error ?? `Server returned ${res.status}`,
          variant: "error",
        });
        return;
  ...
  const handleUseTemplate = async (template: typeof TEMPLATES[0]) => {
    ...
    try {
      ...
      toast({
        title: `Template added: ${template.name}`,
        variant: "success",
      });
      fetchSequences();
    } catch (err) {
  ...
  const handleToggle = async (id: string, isActive: boolean) => {
    try {
      const res = await fetch(`/api/sms/sequences/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        toast({
          title: "Couldn't update sequence",
          description: err?.error ?? `Server returned ${res.status}`,
          variant: "error",
        });
        return;
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        toast({
          title: "Couldn't delete sequence",
          description: err?.error ?? `Server returned ${res.status}`,
          variant: "error",
        });
        return;
  ...
  const handleEnroll = async () => {
    ...
    try {
      const res = await fetch(`/api/sms/sequences/${enrollModal}/enroll`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        toast({
          title: "Couldn't enroll lead",
          description: err?.error ?? `Server returned ${res.status}`,
          variant: "error",
        });
        return;
      }
      toast({ title: "Lead enrolled", variant: "success" });
      setEnrollModal(null);
      setEnrollPhone("");
      setEnrollLeadId("");
      fetchSequences();
