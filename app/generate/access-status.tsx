...
export function GenerateAccessStatus({ onAccess, onSignedIn }: { onAccess: (access: GenerateAccess) => void; onSignedIn: () => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    async function check() {
      ...
      setChecking(true);
      try {
        const r = await fetch("/api/generate/access", { cache: "no-store" });
  ...
  const title = signedIn ? "Signed in · owner access" : status?.code === "MFA_REQUIRED" ? "Finish two-factor authentication" : status?.code === "FORBIDDEN" ? "Owner access required" : status?.code === "SERVICE_UNAVAILABLE" ? "Access check unavailable" : status ? "Sign in to Generate" : "Checking your access…";
  return <section className={`gen-access-status${signedIn ? " is-ready" : ""}`} aria-label="Generate access">
    <div role="status"><strong>{title}</strong><p>{signedIn ? "You can submit generations. Provider availability is shown on each workflow." : status?.error || "Checking your account and generation services."}</p>
      {status?.authenticated === false && <p>Open sign-in below and complete your owner login and authenticator step. Return here; access refreshes automatically and your inputs stay in this tab.</p>}
    </div>
    <div className="gen-top-actions">
      {status?.authenticated === false && <a className="gen-view-toggle" href={status.loginUrl?.startsWith("/login") ? status.loginUrl : "/login?callbackUrl=%2Fgenerate"} target="_blank" rel="noreferrer">{status.code === "MFA_REQUIRED" ? "Complete two-factor ↗" : "Sign in to Generate ↗"}</a>}
