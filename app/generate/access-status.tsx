...
export function GenerateAccessStatus({ onAccess, onSignedIn }: { onAccess: (access: GenerateAccess) => void; onSignedIn: () => Promise<void> }) {
  ...
  return <section className={`gen-access-status${signedIn ? " is-ready" : ""}`} aria-label="Generate access">
    ...
    <div className="gen-top-actions">
      {status?.authenticated === false && <a className="gen-view-toggle" href={status.code === "FORBIDDEN" ? "/logout" : status.loginUrl?.startsWith("/login") ? status.loginUrl : "/login?callbackUrl=%2Fgenerate"} target="_blank" rel="noreferrer">{status.code === "FORBIDDEN" ? "Sign out / switch account ↗" : status.code === "MFA_REQUIRED" ? "Complete two-factor ↗" : "Sign in to Generate ↗"}</a>}
