...
export async function POST() {
  ...
  let scrape;
  try {
    scrape = await fetchStoreOrders("walmart");
  } catch (err) {
  ...
  if (!orders || orders.length === 0) {
    return NextResponse.json(
      { error: "No orders found on the Walmart page" },
  ...
  after(async () => {
    try {
      const result = await ingestOrders(household.id, orders, "Walmart");
      await prisma.foodHousehold.update({
        where: { id: household.id },
        data: { lastWalmartSyncAt: new Date() },
      });
      console.log("[Food] Walmart auto-sync ingested", result);
    } catch (err) {
      console.error("[Food] Walmart auto-sync ingest failed:", err);
    }
  });
