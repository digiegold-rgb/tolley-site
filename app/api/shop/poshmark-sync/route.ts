/**
 * Poshmark inbound mirror sync.
 *
 * Receives a snapshot of Ruthann's Poshmark "My listings" page from the
 * crosslist-poshmark-worker on the DGX and reconciles it against our
 * PlatformListing(platform="poshmark") + Product tables.
 *
 ...
 *     If Poshmark says sold AND the product isn't already sold AND no other
 *     platform has claimed the sale, mark Product.status='sold' + soldAt.
 *   - Unmatched rows are logged but not auto-created — Poshmark isn't the
...
export async function POST(req: NextRequest) {
  ...
  // Collect existing Poshmark listings to short-circuit no-op writes.
  ...
  for (const row of payload.rows) {
    ...
    if (listing) {
      ...
      await prisma.platformListing.upsert({
        where: {
          productId_platform: { productId: product.id, platform: "poshmark" },
        },
        create: {
    ...
    // Promote to product-level sold only if Poshmark says sold and we haven't
  ...
  await prisma.platformAuth.upsert({
    ...
    create: {
      platform: "poshmark",
      profileDir: "/home/jelly/.poshmark-profile",
