/**
 * Depop inbound mirror sync.
 *
 * Receives a snapshot of Ruthann's Depop "My listings" page from the
 * crosslist-depop-worker on the DGX and reconciles it against our
 * PlatformListing(platform="depop") + Product tables.
 *
 ...
 *     If Depop says sold AND the product isn't already sold AND no other
 *     platform has claimed the sale, mark Product.status='sold' + soldAt.
 *   - Unmatched rows are logged but not auto-created — Depop isn't the
...
export async function POST(req: NextRequest) {
  ...
  // Collect existing Depop listings to short-circuit no-op writes.
  ...
  for (const row of payload.rows) {
    ...
    if (listing) {
      ...
      await prisma.platformListing.upsert({
        where: {
          productId_platform: { productId: product.id, platform: "depop" },
        },
        create: {
    ...
    // Promote to product-level sold only if Depop says sold and we haven't
  ...
  await prisma.platformAuth.upsert({
    ...
    create: {
      platform: "depop",
      profileDir: "/home/jelly/.depop-profile",
