...
export async function POST(
  ...
  try {
    const res = await fetch(`${MIROFISH_URL}/simulations/${simId}/interrogate`, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        agent_idx: body.agentIdx,
        question: body.question || "Why did you make that decision?",
      }),
    ...
    return NextResponse.json(
      { error: "MiroFish interrogation unavailable" },
      { status: 503 }
    );
  }
}
