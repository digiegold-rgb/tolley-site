...
type Automation = { enabled: boolean; viewers: { platform:string; peak:number; samples:number; lastAt:number; latest:number; series:{t:number;viewers:number}[] }[]; profileHistory:{followers:number|null;sold:number|null;t:number}[] };
type Whatnot = {publicStatus:string;sellerStatus:string;checkedAt?:number;error?:string;profile?:{handle:string;followers:number|null;sold:number|null;observedAt:number}|null};
type Snapshot = {
  automation: Automation; whatnot: Whatnot;
  sessions: Show[]; activeId: string | null; show: Show | null;
  collector: { at: number | null; error: string; youtube: { connected?: boolean; source?:string; liveNow?:boolean|null; video?: string }; tiktok: { connected?: boolean; source?:string; liveNow?:boolean|null; user?: string; viewers?: number } };
...
export default function Coach() {
  ...
  const ytMismatch = !!show?.sources?.yt && (data?.collector.youtube.source || data?.collector.youtube.video) !== show.sources.yt;
  const ttMismatch = !!show?.sources?.tt && (data?.collector.tiktok.source || data?.collector.tiktok.user) !== show.sources.tt;
  ...
  return <main className="sc">
    ...
    <div className="sc-connections" aria-label="Data connections"><span className={yt ? "connected" : ""}>● YouTube {yt ? "chat connected" : !stale && data?.collector.youtube.liveNow === false ? "not live" : "data unavailable"}</span><span className={tt ? "connected" : ""}>● TikTok {tt ? "chat connected" : !stale && data?.collector.tiktok.liveNow === false ? "not live" : "data unavailable"}</span><span className={data?.whatnot?.publicStatus === "connected" ? "connected" : ""}>● Whatnot · {data?.whatnot?.publicStatus === "connected" ? "public profile connected" : "public profile unavailable"}</span><span>Private guidance · no automatic chat replies</span></div>
    {data && <section className="sc-auto"><div><strong>Automatic data tracking</strong><p>{data.automation?.enabled ? "Verified live shows start tracking automatically. Capture finishes after three minutes of confirmed inactivity." : "Automatic show detection is paused. You can still start and finish tracking manually."}</p></div><button aria-pressed={data.automation?.enabled} disabled={!!busy} onClick={() => void command("automation",{enabled:!data.automation?.enabled})}>{data.automation?.enabled ? "Pause auto detection" : "Enable auto detection"}</button></section>}
    {data?.whatnot && <section className="sc-profile"><div><strong>Whatnot · @treasure_hauls</strong><p>{data.whatnot.profile ? `Public profile observed ${when(data.whatnot.profile.observedAt)}` : "Waiting for a readable public profile"}</p></div><div><b>{data.whatnot.profile?.followers ?? "—"}</b><span>Followers</span></div><div><b>{data.whatnot.profile?.sold ?? "—"}</b><span>Lifetime sold count</span></div><p className="sc-profile-note">{data.whatnot.sellerStatus === "login_required" ? "Private sales sync needs a Whatnot seller login in the collector browser. Public profile counts are separate from show sales." : data.whatnot.error || "Public profile totals are separate from show sales."}</p></section>}
    <section className="sc-session">
      ...
      {!data?.activeId ? <form onSubmit={async e => { e.preventDefault(); const r = await command("start", { title }); if (r) { setTitle(""); changeShow(r.id); } }}><label htmlFor="sc-title">Start manually</label><div className="sc-inline"><input id="sc-title" value={title} onChange={e => setTitle(e.target.value)} maxLength={120} placeholder="Tonight’s treasure haul" required /><button className="sc-primary" disabled={!!busy || !data}>Start tracking</button></div></form> : <div className="sc-track-note">Chat is saved while tracking, even with this page closed. Tracking does not start or stop your broadcast.</div>}
    </section>
    {!data && !loadError && <p role="status" className="sc-empty">Connecting to your coach…</p>}
    {data && !show && <section className="sc-welcome"><span>✦</span><h2>Your next show has a cohost.</h2><p>Your verified YouTube or TikTok live show will start tracking automatically. Connected chat and audience measurements become a saved show history. You can also start tracking manually.</p><p>Add Whatnot sales as they happen. Your original stream controls stay one tap away.</p></section>}
    {show && metrics && <>
      ...
      {!!data.automation?.viewers.length && <section className="sc-viewers">{data.automation.viewers.map(v => <article key={v.platform}><p>{platform[v.platform]} · concurrent viewers</p><strong>{tracking && now-v.lastAt<90 ? v.latest : "—"}</strong><span>Observed peak {v.peak} · {v.samples} samples</span><ViewerChart points={v.series} peak={v.peak}/><small>Last observed {v.latest} · {when(v.lastAt)}. Missing samples stay unknown.</small></article>)}</section>}
...
function ViewerChart({points,peak}:{points:{t:number;viewers:number}[];peak:number}) {
  const first=points[0]?.t||0,last=points.at(-1)?.t||first;
  return <svg viewBox="0 0 320 60" role="img" aria-label="Observed viewer samples over time">{points.map((p,i)=><rect key={i} x={4+(p.t-first)/Math.max(60,last-first)*308} y={55-p.viewers/Math.max(1,peak)*50} width="3" height={Math.max(1,p.viewers/Math.max(1,peak)*50)} rx="1" fill="#d3f7a3"/>)}</svg>;
}
