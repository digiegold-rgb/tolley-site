...
export default async function ProofPage({ params }: Props) {
  ...
  const video = job.videoUrl ?? (job.finalUrl && /\.mp4(\?|$)/i.test(job.finalUrl) ? job.finalUrl : null);
  if (!original || (isBeauty ? !video : !generated)) notFound();
    ...
    >
      <div style={{ maxWidth: 1040, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 16 }}>
          ...
          {generated && !isBeauty && <figure style={{ margin: 0 }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        >
          <p style={{ margin: 0 }}>
            <strong>Disclosure.</strong>{" "}{isBeauty ? "The video was generated with AI to animate the original listing photo with camera motion. Refer to the original photo for the property as photographed." : <>The right-hand image{video ? " and the video were" : " was"} generated with AI from the original photo on the left.
        ...
        <p style={{ fontSize: 14 }}>
          <a
            href="/realestateanimated?utm_source=proof&utm_medium=referral&utm_campaign=listing_self_service#start"
            style={{ color: "inherit", textUnderlineOffset: 3 }}
          >
            Create a photo or video for your own listing
          </a>
        </p>
      </div>
    </main>
