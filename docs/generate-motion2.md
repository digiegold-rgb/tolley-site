...
3. **Dry-run estimate** shows beat count and fal call count **before** spend (`N × seconds × $0.10` @720p). Confirm when remaining spend is over ~$5. Dry run on Go dry-runs kwargs (no GPU) and prints the note on the page.
4. **Go = Run remaining** (sequential last-frame chain), not “beat 1 only forever.” Auto-advance is on by default: after a beat becomes `ready`, the next `draft` is kicked. A 200 without a child job (refused / dry-run / empty) is treated as an error or an explicit dry-run message — never a silent idle. Safety scans the motion prompt only (not the negative, which lists `child` as a reject token).
5. **Queue id** is persisted in `localStorage` + `?queue=`. GET/POST always use that parent id. Latest-among-operators (`shop-admin` / `hq:*`) is only used when there is no bound id — never a silent swap to someone else’s test queue.
6. **Stuck unlock:** a child `generating`/`running` with no fal progress past ~11 min (or fal Completed while the parent stayed generating) is reconciled to done/failed, Generate unlocks, and the UI offers Retry / Dismiss / Continue queue.
7. **Review** each clip. Regenerate a bad beat. **Ripple continuity** (opt-in) re-chains the new last frame into beat k+1 and resets later beats to draft. Without ripple, later clips stay (expect a cut).
8. **Approve** every beat, then **Stitch approved beats**. Concat is `ffmpeg -f concat -c copy` on **Vercel Node** (`maxDuration` 120). Not Spark. Crossfade is not on this path (Motion 1 still has optional two-clip xfade).

The estate-lady cinema short is **Cinema** (`docs/generate-cinema.md`), not Motion 2.
