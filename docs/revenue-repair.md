...
6. The finalizer user-service override now invokes `ops/spark/vater-finalize-sweep.sh` from the release checkout, preserving the existing timer's `--apply` behavior. Direct invocation without arguments remains dry-run. The dry-run passed with two already-ready projects and no charges booked; no immediate apply sweep was invoked during validation.
...
The production additive migration was applied and recorded in Prisma history, and four historical inquiries were recovered. The schema snapshot is private under `/home/jelly/.local/state/tolley-revenue/20260908/`. Finalizer and Buckeye service overrides are installed and the user daemon reloaded. Their previous failed-run status is not evidence of a successful scheduled run; the corrected commands will be used by their existing timers. At this release-preparation checkpoint, the application deployment and financial reconciliation are pending. No customer message or charge was performed by these validations or recovery tools.
