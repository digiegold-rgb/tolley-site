...
The cloud build exhausted its Standard machine's 8 GB memory limit during compilation. A local test with the same total memory cap, no swap, and a four-core CPU quota reproduced the OOM. The build now uses sequential compiler workers, a 4 GB compiler heap to leave room for native allocations, and webpack memory optimization. The Node-only Twilio and Google Analytics SDKs load as installed server packages rather than contributing roughly 700 modules and over 11 MB of generated source to webpack. Verify their built API routes and traced dependency files before deployment. Page generation remains limited to one worker, and hosting resources are unchanged.

Type checking remains a separate mandatory process with an explicit 4 GB heap. The capped reproduction also exposed TypeScript's smaller default heap under a cgroup: it failed around 2 GB before compilation. The cloud failure occurred after type checking, so that is a separate confirmed build issue. Validate the entire capped build and the replacement cloud deployment before calling the deployment repaired.

The corrected capped build passed in 12m47s with 562 prerendered pages and no OOM events. Its cgroup reached the 8 GB limit while reclaiming memory; the result does not establish spare capacity for further growth. Final trace checks found the required Twilio and Google Analytics runtime files, and production-mode API/acceptance checks passed. A successful replacement cloud deployment is still required.
