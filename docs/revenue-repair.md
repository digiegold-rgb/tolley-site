...
6. Install the finalizer wrapper from `ops/spark/vater-finalize-sweep.sh` against the chosen release checkout. Default invocation is dry-run. The existing timer previously supplied `--apply`; restore that scheduled behavior only when the reviewed delivery/usage-booking rollout is intended. The dry-run passed with two already-ready projects and no charges booked.
7. Port the Buckeye script into the worker's selected release. Missing W37 slips still need delivery input; the code does not invent invoice line items.
